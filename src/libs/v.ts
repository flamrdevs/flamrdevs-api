...
export const is_array = <T extends unknown>(value: unknown): value is T[] => Array.isArray(value);
...
export const is_in_object_and_type = <K extends string, T>(
  key: K,
  target: object,
  fn: (value: unknown) => value is T
): target is object & { [key in K]: T } => key in target && fn(target[key as keyof typeof target]);

export const is_optional_in_object_and_type = <K extends string, T>(
  key: K,
  target: object,
  fn: (value: unknown) => value is T
): target is object & { [key in K]: T | undefined } => (key in target ? fn(target[key as keyof typeof target]) : true);

export const is_every = <T>(array: unknown[], fn: (value: unknown) => value is T): array is T[] => array.every(fn);
